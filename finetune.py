...
def test(dataloader, model, loss_fn, device, number_classes):
    model.eval()
    ...
    with torch.no_grad():
        for X, y in dataloader:
            ...
            acc1, acc5 = compute_accuracy(pred, y, topk=(1, number_classes))
...
def main(cfg: argparse.Namespace):
    ...
    # trainset = eval(cfg.DATASET)("./data_set/train/", train=True, transform=train_transforms, download=True)
    # testset = eval(cfg.DATASET)("./data_set/val/", train=False, transform=val_transforms, download=True)

    # trainset = ("./data_set/train/")
    # testset = ("./data_set/val/")
    ...
    number_classes = len(trainset.classes)
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        if (epoch+1) % cfg.EVAL_INTERVAL == 0 or (epoch+1) == cfg.EPOCHS:
            top1_acc, top5_acc = test(testloader, model, val_loss_fn, device, number_classes )

            if top1_acc > best_top1_acc:
                best_top1_acc = top1_acc
                best_top5_acc = top5_acc
                torch.save(model.state_dict(), save_dir / f"{cfg.MODEL}_{cfg.VARIANT}_{trainset.__class__.__name__}.pt")
